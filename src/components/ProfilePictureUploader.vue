<template>
  ...
  />
  <q-banner
    class="rounded-borders flex"
    @dragover="dragover"
    @dragleave="dragleave"
    @drop="drop"
    :class="{ 'bg-grey-4': dropboxActive, 'bg-grey-3': !dropboxActive }"
  >
    <q-avatar
      color="secondary"
      text-color="white"
...
export default {
  ...
  mounted() {
    this.$nextTick(function () {
      ...
      getDownloadURL(
        ref(
          storage,
          "profileImages/" + this.$store.state.auth.data.uid + "_128x128"
        )
      )
        .then((url) => {
  ...
  methods: {
    ...
    deleteFile() {
      ...
      const storageRef32 = ref(
        storage,
        "profileImages/" + this.$store.state.auth.data.uid + "_32x32"
      );
      const storageRef64 = ref(
        storage,
        "profileImages/" + this.$store.state.auth.data.uid + "_64x64"
      );
      const storageRef128 = ref(
        storage,
        "profileImages/" + this.$store.state.auth.data.uid + "_128x128"
      );
    ...
    uploadFile() {
      const storageRef = ref(
        storage,
        "profileImages/" + this.$store.state.auth.data.uid
      );

      this.uploadLoading = true;

      // 'file' comes from the Blob or File API
      uploadBytes(storageRef, this.uploadedFile)
        .then((snapshot) => {
          this.$q.notify({
