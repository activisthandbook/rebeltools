<template>
  <span class="cursor-pointer non-selectable">
    ...
    <q-btn round dense unelevated text-color="black" color="white" class="q-mx-sm">
      <q-avatar size="32px" style="margin:1px">
        <q-icon name="mdi-account" v-if="!profileImage2x" size="24px"/>
        <img :src="profileImage2x" :srcset="profileImage1x + ' 1x,' + profileImage2x + ' 2x'" no-spinner v-else />
      </q-avatar>
    </q-btn>
...
export default {
  ...
  mounted () {
    this.$nextTick(function () {
      getDownloadURL(ref(storage, 'profileImages/' + this.$store.state.auth.data.uid + '_32x32'))
        .then((url) => {
